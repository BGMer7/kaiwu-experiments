...
def reward_shaping(
    ...
    # 奖励4.1 撞墙闪现的惩罚 (TODO)
    reward_flicker_bump = 0
    is_flicker_bump = bump(curr_pos_x, curr_pos_z, prev_pos_x, prev_pos_z)
    if is_flicker_bump:
        reward_flicker_bump += 200

    # Reward 4.2 Reward for normal flickering (TODO)
    # 奖励4.2 正常闪现的奖励 (TODO)
    if not is_flicker_bump:
        reward_flicker += 50

    # Reward 4.3 Reward for super flickering (TODO)
    # 奖励4.3 超级闪现的奖励 (TODO)
    if cul_dist(curr_pos_x, curr_pos_z, prev_pos_x, prev_pos_z) >= 7500:
        reward_flicker += 200
    ...
    reward = [
        ...
        reward_flick_bump * float(REWARD_CONFIG["reward_flicker_bump"]),
        reward_flick * float(REWARD_CONFIG["reward_flicker"]),
    ]
